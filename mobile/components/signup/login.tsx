...
const SignupLogin = ({
  ...
  // Check if passwords match for visual feedback — only show after blur or submit attempt
  const shouldShowFeedback = (confirmPasswordTouched || submitAttempted) && confirmPassword;
  const passwordsMatch = shouldShowFeedback && data.Password === confirmPassword;
  const showPasswordMismatch = shouldShowFeedback && data.Password !== confirmPassword;
